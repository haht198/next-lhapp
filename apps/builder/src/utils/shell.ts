...
export const executeCommand =  (command: string, dir?: string, stdoutReturn?: boolean) => {
  ...
  export const spawnCommand = (command: string, args: string[]): Promise<number> => {
    return new Promise((resolve, reject) => {
      ...
      signProcess.on('exit', (code, signal) => {
        console.debug(`[Utils] Spawn command "${command}" exit: ${code} ${signal}`);
      ...
      signProcess.stderr.on('data', (data) => {
        console.debug(`[Utils] Spawn command "${command}" stderr: ${data.toString('utf8').trim()}`);
      });
    });
  }
