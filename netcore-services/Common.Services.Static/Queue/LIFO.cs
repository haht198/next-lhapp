...
{
    public class LIFO<TItem> : IQueue<TItem>
        where TItem : IQueueItem
    {
        ...
        public LIFO(QueueSetting<TItem> _setting = null)
        {
        ...
        public void Push(TItem item)
        {
            lock (items)
            {
                ...
                {
                    items.Remove(items.First(t => t.Item.ItemId == item.ItemId));
                }
                items.Insert(0, new QueueItemProcessing(item));
            }
            DoProcessQueue();
        }
